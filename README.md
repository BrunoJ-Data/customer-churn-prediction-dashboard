# 📊 SaaS Churn Prediction Dashboard

An interactive dashboard designed for SaaS Customer Success and Sales teams to identify churn risks, analyze associated profits and losses, and take proactive retention actions.

👉 **Live App:** [customer-churn-prediction-dashboard-interactive.streamlit.app](https://customer-churn-prediction-dashboard-interactive.streamlit.app)

*Detailed README coming soon...*

---

:speech_balloon:  Un tableau de bord interactif afin d'anticiper les départs clients et les risques budgétaires associés.

*Documentation complète bientôt disponible...*

 

## ⚙️ Preparation / Cleaning
- Données issues csv issues de " Ravenstack SaaS Subscription & Churn Analytics "
- Nettoyage, structuration et agrégation de jeux de données (feature engineering, gestion des valeurs manquantes, encodage) avec Pandas .
- Consolidation et jointures des données clients sous Pandas.

## 🛠️ Stack
- Python, Pandas, NumPy, Scikit-Learn
- **Interface:** Streamlit
